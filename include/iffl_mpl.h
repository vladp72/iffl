...
//!
//! @brief This module contains few metaprogramming facilities that
...
//!
//! @namespace iffl::mpl
//! @brief intrusive flat forward list metaprograming helpers library
//!
namespace iffl::mpl {
...
//! @brief intrusive flat forward list metaprograming helpers library internal types
//!
namespace details {
    //!
    //! @class detector
    //!
    //! @brief defines detector metafunction
    //!
    //! @tparam Default - Default type assigned to detector::type is SFINAE 
    //!                   fails instatiation of Operation<Arguments>
    //! @tparam AlwaysVoid - used by specialization as place where it will 
    //!                      attempt instatiation of Operation<Arguments>.
    //!                      It is not used in default case
    //! @tparam Operation - metafunction that we are trying to evaluate
    //! @tparam Arguments - arguments to be passed to the metafunction
    //!
    ...
    //!         and detector::type is Opration<Arguments...>
    //!
    ...
    struct detector<Default,
        ...
        //!
        //! @brief In this template specialization it is 
        //! type of metafunction Operation<Arguments...>
        //!
...
//! is well formed
//! @tparam Operation - metafunction that we will validate for being well formed given the Arguments
//! @tparam Arguments - template arguments that will be used to instantiate metafunction Operation 
...
//! @tparam Arguments - template arguments that will be used to instantiate metafunction Operation 
//!                     that is a template template parameter
//! @return Returns detected_t will be either nonesuch or result of metafunction Operation<Arguments...>,
//!         depending if Operation<Arguments...> metafunction is well formed
//!
template <template <typename... > typename Operation,
          typename... Arguments>
using detected_t =
    typename details::detector <nonesuch,
                                ...
                                Arguments...>::type;
//!
//! @brief Instance of detected_t metafunction output
//! @tparam Operation - metafunction that we will validate for being well formed given the Arguments
//! @tparam Arguments - template arguments that will be used to instantiate metafunction Operation 
//!                     that is a template template parameter
//! @return Result of metafunction Operation<Arguments...> if Operation<Arguments...> metafunction is well formed
//!         otherwise illformed because metafunction returns nonesuch, which cannot be instantiated
//!
template <template <typename... > typename Operation,
          typename... Arguments>
...
//! @tparam Arguments - template arguments that will be used to instantiate metafunction Operation 
//!                     that is a template template parameter
//! @return Returns type of detector for metafunction Operation<Arguments...> if this expression is well formed,
//!         otehrwise it returns detector with Default
//!
template <typename Default,
          template <typename... > typename Operation,
          typename... Arguments>
...
//! @tparam Arguments - template arguments that will be used to instantiate metafunction Operation 
//!                     that is a template template parameter
//! @return Returns type of metafunction Operation<Arguments...> if this expression is well formed,
//!         otehrwise it returns Default
//!
template <typename Default,
          template <typename... > typename Operation,
          typename... Arguments>
...
//! @brief returns std::true_type when result of metafunction detected_t is the same as ExpectedType
//! @tparam ExpectedType - type that we will compare result of detected_t to
//! @tparam Operation - metafunction that we will validate for being well formed given the Arguments
//! @tparam Arguments - template arguments that will be used to instantiate metafunction Operation 
//!                     that is a template template parameter
//! @return Returns std::true_type if result of metafunction ExpectedType is the same as ExpectedType,
...
//! @tparam ConvertableToType - type that we will should attempt to convert result of detected_t to.
//! @tparam Operation - metafunction that we will validate for being well formed given the Arguments
//! @tparam Arguments - template arguments that will be used to instantiate metafunction Operation 
//!                     that is a template template parameter
//! @return Returns std::true_type if result of metafunction detected_t is convertable to ConvertableToType,
//!         and std::false_type otherwise
//!
...
using is_detected_convertable =
            std::is_convertible<detected_t<Operation,
                                ...
                                ConvertableToType>;
//!
//! @brief instance of the is_detected_convertable
//! @tparam ConvertableToType - type that we will should attempt to convert result of detected_t to.
//! @tparam Operation - metafunction that we will validate for being well formed given the Arguments
//! @tparam Arguments - template arguments that will be used to instantiate metafunction Operation 
//!                     that is a template template parameter
